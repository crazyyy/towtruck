...
var formsAppClk = {
  showLightbox ( id, target ) {
    $f( formsApp.loaded[id] ).closest( '.ba-form-lightbox-layout' )
      .each( function() {
  ...
  click ( target ) {
    const matches = target.className.match( /ba-click-lightbox-form-\d+/ ) || target.href.match( /ba-click-lightbox-form-\d+/ ),
    ...
    if ( loaded && formsApp.loaded[id] ) {
      ...
      xhr.onload = xhr.onerror = function() {
        ...
        if ( xhr.readyState == 4 && text != '' ) {
          ...
          div.querySelectorAll( 'style, link' ).forEach( ( el ) => {
